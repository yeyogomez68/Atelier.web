...
public class StockMaterialesManagedBean implements Serializable {
    ...
    public void ingresarInsumo(Ordencompradeta ordenCompraDeta) {
        ...
        if (ordenCompraDetaEJB.setActualizarEstadoOrderCompraDeta(ordenCompraDeta, EstadoEnum.APROBADO.getId())) {
            Comunes.mensaje((stockMaterialesEJB.setModificarStockMaterial(ordenCompraDeta.getMaterialId(), Integer.parseInt(String.valueOf(ordenCompraDeta.getOrdenCompraCantidad()))) ? "Se ha ingresado el insumo correctamente " : "Error ingresando el insumo"), ordenCompraDeta.getMaterialId().getMaterialNombre());
            actualizarLista();
        } else {
    ...
    public void ingresarTodos() {
        ...
        for (Ordencompradeta ocd : ordenCompras) {
            if (ordenCompraDetaEJB.setActualizarEstadoOrderCompraDeta(ocd, EstadoEnum.APROBADO.getId())) {
                Comunes.mensaje((stockMaterialesEJB.setModificarStockMaterial(ocd.getMaterialId(), Integer.parseInt(String.valueOf(Math.round(ocd.getOrdenCompraCantidad())))) ? "Se ha ingresado el insumo correctamente " : "Error ingresando el insumo"), ocd.getMaterialId().getMaterialNombre());
            } else {
                Comunes.mensaje("No es posible ingresar el insumo", "");
            }
        }
